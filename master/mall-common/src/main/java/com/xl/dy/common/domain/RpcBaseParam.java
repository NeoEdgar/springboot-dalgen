package com.xl.dy.common.domain;

import com.xl.dy.common.enumm.RpcSignTypeEnum;
import com.xl.dy.common.util.*;
...
/**
 * 服务接口调用入参基类 Created by admin on 2016/4/27.
 */
public class RpcBaseParam implements Serializable {
	...
	/**
	 * 签名计算方法 参见RpcSignTypeEnum 0: 明文 1: SHA-1
	 */
	protected Integer rpcSignType;
	/**
	 * 签名(用于验证调用方的合法性) 签名计算方法:
	 * 签名方法(key(由RPC服务端分配,不在通讯中传递)+scrSysId+rpcDateTime(yyyyMMddHHmmssSSS)+rpcSignType+bizSeqNo+bizSign),如果字段为null则不参与
	 ...
	 * eg.
	 * Constant.MP_BIZ_SEQUENCE_NO_PREFIX)+DateUtils.getCurrentTimeStr("yyyyMMddHHmmss")
	...
	public RpcBaseParam(String rpcSrcSysId, String rpcSignKey, String bizSeqNoPrefix) {
		...
		decriptBuffer.append(rpcSignKey).append(this.rpcSrcSysId).append(this.rpcDateTime).append(this.rpcSignType)
	...
	public RpcBaseParam(String rpcSrcSysId, String rpcSignKey, String bizSeqNoPrefix, String bizSign) {
		...
		StringBuffer decriptBuffer = new StringBuffer();
		decriptBuffer.append(rpcSignKey).append(this.rpcSrcSysId).append(this.rpcDateTime).append(this.rpcSignType)
				.append(this.bizSeqNo).append(this.bizSign);
		this.rpcSign = RpcSignUtils.sha1(decriptBuffer.toString());
	}
	...
	/*
	 * public Map<String, Object> convert2Map() { if (this == null) { return null; }
